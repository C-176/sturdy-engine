...
sql = 'alter table baiyangwantest add (水源地日期 varchar(255),水源地时间 time(0))'
query_write(sql)
sql = 'alter table baiyangwantest add (出水日期 varchar(255),出水时间 time(0))'
query_write(sql)
sql = 'select * from baiyangwantest'
...
for i in data:
    ...
    time_cha1 = 14500 / (water1 / 3.0772)  # 根据公式计算滞后时间1（从水源地到水厂之间的时间），单位是（小时）
    if water2 != 0:
    ...
    sql = "update baiyangwantest set 出水时间='%s',出水日期='%s',水源地时间='%s',水源地日期='%s' where id = %d" \
