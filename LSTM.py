...
# 导入酒精销售数据
df = pd.read_csv(r'C:\Users\1\Desktop\Python源码\06_深度学习\RF\csv\baiyangwan.csv',index_col='id',parse_dates=['原水测量日期'],nrows=1000)
print(df.head(2))
name = '原水浊度'
# 观察数据集，这是一个单变量时间序列
...
print(len(train_data))  # 等于300（原始数据集长度）-12（测试集长度）-12（时间窗口）
# 观察某一个数据点
print(train_data[0])
...
plt.grid(True)
plt.plot(df[name])
# x = np.arange('2019-11-11', '2019-11-13', dtype='datetime64[M]').astype('datetime64[D]')
x = range(950,1000)
plt.plot(x,true_predictions,'r')
plt.show()
#
# # 放大看
